<template>
  <section class="relative h-48 md:h-96 bg-cover bg-center flex items-center justify-center"
    ...
    <div class="relative text-center text-white px-4">
      <h1 class="text-2xl md:text-4xl font-bold">Webinar Details</h1>
      <nav class="mt-2 text-xs md:text-sm">
        <ul class="flex items-center justify-center gap-1 md:gap-2 flex-wrap">
          <li>
            <a href="/" class="hover:underline">Home</a>
            > <span class="text-gray-400">Webinar Details</span>
          </li>
        </ul>
      </nav>
    </div>
  </section>
  <section class="max-w-full bg-[#F9F7F1] mx-auto p-8">
    <div class="flex flex-col md:flex-row md:space-x-8">
      <!-- Left: Banner Image (70%) -->
      <div class="md:w-[70%]">
        <img :src="bannerImage" alt="Webinar Banner" class="w-full h-auto rounded-lg object-cover" />
        <!-- Title -->
        <h1 class="mt-8 text-3xl font-bold text-gray-900">{{ title }}</h1>
        <!-- Description -->
        <p class="mt-4 text-gray-700" v-html="formattedDescription"></p>

        <!-- Meet the Speaker -->
        <div class="mt-8 border rounded-lg p-6 shadow-sm">
          <h2 class="text-2xl font-semibold mb-4">Meet the Speaker</h2>
          <div class="flex items-center space-x-4">
            <img :src="speaker.image" alt="Speaker Image" class="w-20 h-20 rounded-full object-cover" />
            <div>
              <p class="text-lg font-medium">{{ speaker.name }}</p>
              <p class="text-sm text-gray-500">{{ speaker.designation }}</p>
              <div class="mt-2 flex space-x-3">
                <a v-for="link in speaker.socialLinks" :key="link.name" :href="link.url" target="_blank"
      ...
      <div class="md:w-[30%] mt-6 md:mt-0 flex flex-col gap-6">
        <!-- Info Box 1 -->
        <div class="bg-gray-50 rounded-lg p-6 shadow-md flex flex-col">
          <h2 class="text-2xl font-semibold mb-4">Information</h2>
          <div class="w-16 h-1.5 bg-[#fd620e] rounded mb-4"></div>
          <ul class="space-y-4 divide-y divide-gray-200">
            ...
            <!-- <li class="pt-4">
              <strong>Duration:</strong> {{ webinarDetails.duration }}
            </li> -->
            <li class="pt-4">
              <strong>Platform:</strong> {{ webinarDetails.platform }}
            </li>
          </ul>
          <!-- <a :href="webinarDetails.registrationLink" target="_blank" rel="noopener noreferrer"
            class="mt-6 inline-block bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700 text-center">
            Register Now
          </a> -->
        </div>

        <!-- Info Box 2 (Duplicate) -->
        <div class="bg-gray-50 rounded-lg p-6 shadow-md flex flex-col">
          ...
          <div class="mt-2">
            <button type="submit" @click="openWhatsApp"
...
const openRegistrationLink = () => {
  const url = webinarDetails?.registrationLink
  if (url) {
...
const openWhatsApp = () => {
  ...
  const message = `Hello! I would like to register for the upcoming webinar. Please share the details.`
